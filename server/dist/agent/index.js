...
class AgentOrchestrator {
    ...
    async addTrace(type, decision, evidence, nextAction) {
        const timestamp = new Date();
        await (0, tenant_1.runWithTenant)(this.merchantId, async () => {
            await models_1.AgentRun.updateOne({ runId: this.runId }, {
                $push: {
    ...
    async addToolCall(toolName, inputs, outputs, error = null) {
        await (0, tenant_1.runWithTenant)(this.merchantId, async () => {
            await models_1.AgentRun.updateOne({ runId: this.runId }, {
                $push: {
    ...
    async executeCycle(goal) {
        try {
            // 1. INVESTIGATING State
            await this.updateStatus('investigating');
            await this.addTrace('decision', 'Initiating transaction analysis to understand baseline conversions.', `Merchant specified metric target: ${goal.metric} uplift of ${goal.targetUpliftPercent}%.`, 'get_transaction_metrics');
            // Execute Tool 1: get_transaction_metrics
            const txMetrics = await this.safeExecuteTool('get_transaction_metrics', () => tools.get_transaction_metrics(this.merchantId, { timeframeDays: 30 }), { timeframeDays: 30 });
            ...
            await this.addTrace('decision', `Transaction success rate is ${successRate}%. Querying payment failures to analyze drop-offs.`, `Out of ${totalAttempts} attempts, ${failedCount} failed.`, 'analyze_payment_failures');
            // Execute Tool 2: analyze_payment_failures
            const failuresAnalysis = await this.safeExecuteTool('analyze_payment_failures', () => tools.analyze_payment_failures(this.merchantId, { timeframeDays: 7 }), { timeframeDays: 7 });
            ...
            await this.addTrace('decision', `Isolating affected customer cohorts based on gateway timeout anomalies: ${severityText}.`, `Identified main gateway downtime on ${anomaly ? anomaly.gateway : 'SBI'} UPI payments.`, 'get_customer_segments');
            // Execute Tool 3: get_customer_segments
            const segmentsData = await this.safeExecuteTool('get_customer_segments', () => tools.get_customer_segments(this.merchantId, {}), {});
            // Execute Tool 4: get_campaign_history to review weights
            await this.safeExecuteTool('get_campaign_history', () => tools.get_campaign_history(this.merchantId, {}), {});
            // 3. SIMULATING State
            await this.updateStatus('simulating');
            await this.addTrace('decision', 'Running growth simulator for proposed Payment Fallback Strategy against checkout recovery options.', 'Targeting segment high_value_upi_users with a 5% card/netbanking fallback promotion.', 'run_growth_simulation');
            ...
            const simResultA = await this.safeExecuteTool('run_growth_simulation', () => tools.run_growth_simulation(this.merchantId, simArgsA), simArgsA);
            ...
            const simArgsB = {
                strategyType: 'checkout_recovery',
                segmentKey: 'high_value_upi_users',
                parameters: {
            ...
            const simResultB = await this.safeExecuteTool('run_growth_simulation', () => tools.run_growth_simulation(this.merchantId, simArgsB), simArgsB);
            ...
            const chosenStrategy = isLowBudget ? 'checkout_recovery' : 'payment_method_fallback';
            ...
            await this.addTrace('decision', isLowBudget
                ? 'Enqueuing zero-cost Checkout Recovery Link campaign to approval queue due to strict budget constraints.'
                : 'Enqueuing Card Fallback Offer campaign to human approval queue.', `Strategy A projected ROI is ${roiA} (Uplift: +${upliftA}%). Strategy B ROI is ${roiB} (Uplift: +${upliftB}%). Chosen strategy is ${chosenStrategy}.`, 'create_razorpay_action');
            ...
            const actionResult = await this.safeExecuteTool('create_razorpay_action', () => tools.create_razorpay_action(this.merchantId, actionArgs, this.runId), actionArgs);
